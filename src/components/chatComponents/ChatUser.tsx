...
function ChatUsers({
  ...
  useEffect(() => {
    setSortedConversations(sortConversations(conversations));
  }, [conversations]);
    ...
    addConversation({ senderId, receiverId })
      .then((response: any) => {
        ...
        const existChat = conversations.filter(
          (con: any) => con._id === conversationData._id
        );
        if (!existChat.length) {
          setConversations((prev: any) => {
            const newConversations = [...prev, conversationData];
            setSortedConversations(sortConversations(newConversations));
  ...
  return (
    <div
      className="relative flex flex-col  h-full bg-primary border-r border-gray-300  md:block transform transition-all duration-500 ease-in-out"
      style={{ width: "24rem" }}
    >
              ...
              <div key={conversation._id} onClick={() => setCurrentChat(conversation)}>
                <Friend
                  ...
                  updateConversation={(updatedConversation) => {
                    setConversations((prev: any) => {
                      const newConversations = prev.map((conv: any) =>
      ...
      <div className="fixed bottom-0 right-0 z-40 mb-6 mr-4">
        <button
          ...
          className="flex items-center justify-center w-10 h-10 mr-3 text-xl font-semibold text-white  bg-green-600 rounded-full focus:outline-none flex-no-shrink"
        >
          <PlusCircle size={18} />
        </button>
      </div>
        ...
        <Modal show={openModal} size="md" onClose={() => setOpenModal(false)}>
          <Modal.Body>
            <div className="flex justify-between items-center">
              <p className="text-sm font-semibold dark:text-white">Add User</p>
            ...
            <div className="w-full">
              <div className="relative flex items-center w-full pl-2 overflow-hidden text-gray-600 focus-within:text-gray-400">
                ...
                <input
                  type="search"
                  name="q"
                  className="w-full m-1 py-2 pl-12 text-xs border-none  bg-gray-200 items-center h-10  pr-4 rounded-md focus:border-gray-200  focus:ring-1 focus:ring-offset-0 focus:ring-green-600 transition-colors duration-300"
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="truncate text-xs font-medium  text-gray-900 dark:text-white">
                                {user?.profile?.fullname ||
                                  user?.companyProfile?.fullname}
                              </p>
                              <p className="truncate text-xs text-gray-500 dark:text-gray-400">
                                {user?.profile?.designation ||
                                  user?.companyProfile?.companyType}
                              </p>
                            </div>

                            <button
                              onClick={() =>
                                handleAddConversation(userId, user._id)
                              }
